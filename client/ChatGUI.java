...
public class ChatGUI extends JFrame implements ChatClient.ChatEventListener {
    ...
    private static final String GROUP_TARGET = "🌐 Everyone (Group Chat)";
    ...
    private JPanel createLoginPanel() {
        ...
        // Server Host & Port
        c.gridy = 2; c.gridx = 0;
        card.add(new JLabel("Server Host:"), c);
        hostField = new JTextField("localhost", 14);
        c.gridx = 1;
        card.add(hostField, c);

        c.gridy = 3; c.gridx = 0;
        card.add(new JLabel("Port:"), c);
        portField = new JTextField("12345", 14);
        c.gridx = 1;
    ...
    private JPanel createChatPanel() {
        ...
        userJList.addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                if (val == null || val.equals(GROUP_TARGET)) {
                    ...
                    targetLabel.setText("Sending to: 🌐 Everyone (Group Broadcast)");
                    targetLabel.setForeground(new Color(25, 118, 210));
                } else {
                    // Extract username (strip any bullet or status indicator)
                    String cleaned = val.replace("🟢 ", "").replace(" (You)", "").trim();
                    selectedTarget = cleaned;
                    targetLabel.setText("Sending to: 🔒 " + cleaned + " (Private Message)");
        ...
        userScroll.setBorder(BorderFactory.createTitledBorder("Active Contacts"));
        ...
        bottomPanel.setBorder(BorderFactory.createEmptyBorder(6, 4, 4, 4));

        targetLabel = new JLabel("Sending to: 🌐 Everyone (Group Broadcast)");
